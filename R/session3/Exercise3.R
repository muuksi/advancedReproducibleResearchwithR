...
library(formattable)
#read in data
my_data<-read_csv("R/session3/IST_corrected.csv")
...
######
#break_seq_low <- c(0, 51, 60, 70, 80)
break_seq_high <- c(50, 59, 69, 79, 120)

#name the categories
#names_seq<-NULL
#for (i in 1:length(break_seq_low))
#  age_seq<-c(names_seq,paste(break_seq_low[i],"-",break_seq_high[i],sep=""))

age_seq <- c("<50","50–59", "60–69", "70–79", ">80")
table_age <- 
  my_data %>% 
  mutate(category=cut(AGE, breaks = c(0, break_seq_high), labels = age_seq)) %>%
  group_by(category) %>%
